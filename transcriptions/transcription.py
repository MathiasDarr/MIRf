import librosa
from transcriptions.transcription_utils import
    ...
    def generate_transcription_json(self):
        ...
        for m, measure in enumerate(self.measures):
            ...
            for i in range(len(notes)):
                note_dictionary = {'measure': str(m), 'midi': str(notes[i][2]), 'string': str(notes[i][3]),
                                   'beat': str(beats[i])}
...
class Measure:
    def __init__(self, notes, index):
        ...
        note_durations = np.array([sixteenth_note_duration, sixteenth_note_duration * 2, sixteenth_note_duration * 4,
                                   sixteenth_note_duration * 6, sixteenth_note_duration * 8,
                                   sixteenth_note_duration * 12,
